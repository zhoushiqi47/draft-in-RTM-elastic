...
\be
\hat
T_M(\xi,x_2;y_2)&=& \frac{\mathrm{i}}{\omega^2 \gamma(\xi)} \Bigg\{ E(\xi)e^{\mathrm{i}\mu_s(x_2+y_2)}+F(\xi)e^{\mathrm{i}\mu_p(x_2+y_2)}\\ \nn
...
In this section we introduce RTM method for inverse elastic scattering problems in the half space. Assume that there $N_s$ sources and $N_r$ receivers unifomly distributed on $\Gamma^d_0$, where $\Gamma^d_0=\{(x_1,x_2)^T\in\Gamma_0:x_1\in[-d,d]\},d>0$ is aperture. We denote by $\Omega$ the smapling domain in which the obstacle is sought. Let $h=dist(\Omega,\Gamma_0)$ be the distance of $\Omega$ to $\Gamma_0$. We assume the obstacle $D\subset\Omega$ and there exist constants $0<c_1<1,c_2>0,c_3>0$ such that
\be\label{o_bound}
...
\end{lem}
\debproof
We define $R_z(f)=u_z$, then it is easy to see that $R_z$ is a linear bounded operator from $L^2_c(\R^2_+)$ to $H^{1,-s}(\R^2_+)$, where $L^2_c(\R^2_+)$ is denote by all $ f  \in L^2 (\R^2_+) $ with compact support in $B_R$. It follows from theorem 3.7 in \cite{Yves1988} that $R_z$ is a uniformly continuous operator continues valued function on $z=1+\i\varepsilon$, $0<\varepsilon<1$ with value in $B(L^2_c(\R^2_+),H^{1,-s}(\R^2_+))$. Then, we can obtain that $R_z$ is uniformly bounded in $B(L^2_c(\R^2_+),H^{1,-s}(\R^2_+))$. This complete the proof by the defintion of the operator norm.
\finproof
