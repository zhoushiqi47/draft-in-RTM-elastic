...
\title[]{Elastic Problem in Half-space}
...
The desired Green function should be obtained by taking the inverse Fourier transform of $\hat G(\xi,x_2;y_2)$. Unfortunately, one cannot simply take the inverse Fourier transform in the above formula because $\delta(\xi)$ have zero points in the real axis by lemma \ref{lem2.1} \cite{achenbach1980}.
...
&&-A(\xi)e^{\mathrm{i}\mu_s x_2+\mu_p y_2}-B(\xi)e^{\mathrm{i}\mu_p x_2+\mu_s y_2}\Bigg\}e^{\mathrm{i}\xi(x_1-y_1)}
\ee
where
\ben
    &&{A(\xi)} =
...
\ee
Let $T_D(x,y)$ denote the traction of $D(x,y)$ in direction $e_2$ to variable $x$ that $T_D(x,y)e_i=T_x^{e_2}(D(x,y))e_i=T_x^{e_2}(D(x,y)e_i)$. Then we can get the representation of $T_D(x,y)$ by a trivial caculation.
\be
...
&&-E(\xi)e^{\mathrm{i}\mu_s x_2+\mu_p y_2}-F(\xi)e^{\mathrm{i}\mu_p x_2+\mu_s y_2}\Bigg\}e^{\mathrm{i}\xi(x_1-y_1)}
\ee
where
\ben
		&&{E(\xi)} =
...
\hspace{-2cm}
\hat
    T_D(\xi,0;y_1,y_2)=\frac{1}{\gamma(\xi)}\Bigg[\Bigg(   \begin{array}{cc}
	\mu_s\mu_p & \xi\mu_p \\
	\xi\mu_s & \xi^2
...
Let $f(\xi,\mu_s,\mu_p)=g(\xi,\mu_s,\mu_p)/\gamma(\xi)$ where g(x,y,z) is a  quadratic polynomial with respect to x,y,z. Let $a,b>0$ and $\rho=\sqrt{a^2+b^2}$. Assume $a/\rho >(1+\kappa)/2$, $b/\rho < \kappa/2$ and $k_s\rho >1$, then we have
\be
\Big|\int_\R f(\xi,\mu_s,\mu_p) e^{\i(\mu_s b +\xi a)}\Big| \leq C(\frac{k_s^2 b}{\rho(k_s\rho)^{1/2}}+\frac{k_s^2a}{\rho(k_s\rho)^{3/2}})
\ee
where C is only dependent on $\kappa$.
\end{lem}
\debproof
Let I(a,b) denote the integral in the left side of inequality (\ref{es_dgreen}). To simplify the integral, the standard substitution $\xi=k_s\sin t$ is made, taking the $\xi$-plane to a strip $-\pi/2<\Re t <\pi/2$ in the t-plane, and the real axis in the $\xi$-plane onto the path L from $-\pi/2+\i\infty\rightarrow-\pi/2\rightarrow\pi/2\rightarrow\pi/2-\i\infty$ in the t-plane. The integral I(a,b) then becomes(Let a=$\rho \sin\phi$  and b=$\rho\cos\phi$)
\be
 k_s^2 \int_L f(\sin t,\cos t,(\kappa^2-\sin^2 t)^{1/2})\cos t \ e^{\i k_s\rho(\cos (t-\phi))} dt
\ee
Taking the shift transformation of t and using cauchy integral theorem, we can obtain the representation of I(a,b):
\ben \hspace{-1.5cm}
k_s^2\int_L f(\sin (t+\phi),\cos (t+\phi),(\kappa^2-\sin^2 (t+\phi))^{1/2})\cos (t+\phi) e^{\i k_s\rho(\cos t)} dt \\\hspace{-2cm}
=k_s^2\cos \phi \int_L f(\sin (t+\phi),\cos (t+\phi),(\kappa^2-\sin^2 (t+\phi))^{1/2})\cos t \ e^{\i k_s\rho(\cos t)} dt \\\hspace{-2cm}
-k_s^2\sin \phi \int_L f(\sin (t+\phi),\cos (t+\phi),(\kappa^2-\sin^2 (t+\phi))^{1/2})\sin t \ e^{\i k_s\rho(\cos t)} dt \\\hspace{-2cm}
:=k_s^2(\cos\phi \ I_1+\sin\phi \ I_2)
\een
...
Moreover, because $f$ and $\pa f/\pa t$ has on singularity on $L_2$ and $\i\cos t \to -\infty$ as $t\to \pm\infty$ along $L_2$, it is easy to see that $|I_{12}|\leq C/(k_s\rho)$ via integration by parts.

For $I_2$, using integration by parts on path $L$ first, we have
\be \hspace{-2cm}
I_2=\frac{k_s}{\i\rho} \int_L f(\sin (t+\phi),\cos (t+\phi),(\kappa^2-\sin^2 (t+\phi))^{1/2}) d \ e^{\i(k_s\rho \cos t)} \\ \hspace{-1.5cm}
=\frac{k_s}{\i\rho} \int_{L_1 \cup L_2} \frac{\pa f(\sin (t+\phi),\cos (t+\phi),(\kappa^2-\sin^2 (t+\phi))^{1/2})}{\pa t
} \  e^{\i(k_s\rho \cos t)} dt \\ \hspace{-1.5cm}
=\frac{k_s}{\i\rho}(I_{21}+I_{22})
\ee
...
\ee
where $h$ and $\pa h/\pa t$ are integrable on path $L_1$. By the assumption above, there exist $0<\delta<\pi/4$ only dependent on $\kappa$ such that $|\sin(t+\phi)|>(1+\kappa)/2, |\cos(t+\phi)|<\kappa/2$ for any $t\in(-\delta,\delta)$ while $|\cos(t+\phi)|>(1+\kappa)/2, |\sin(t+\phi)|<\kappa/2$ for any $t\in(-\pi/2,-\pi/2+\delta)\cup(\pi/2-\delta,\pi/2)$. Let define $t_1,t_2\in \chi_1=(-\pi/2+\delta,-\delta)\cup(\delta,\pi/2-\delta)$ which satisfy $\kappa^2 = \sin^2 (t_i+\phi)$, $i=1,2$. Moreover, for any $0<a<1$ and $1<b<1/\kappa$, there exists $\sigma>0$, which satisfy that $\chi_2=(t_1-\sigma,t_1+\sigma)\cup(t_2-\sigma,t_2+\sigma)\subset\chi_1$ and is only dependent on a,b,$\kappa$, such that
\be \label{assume1}
a\kappa<|\sin (t+\phi)|<b\kappa.
\ee
for any $t\in\chi_2$. We are now in a position to estimate $I_{21}$. Similarly, we split the path $L_1$ into $\chi_2$ and $L_1\bks \chi_2$, then we have the corresponding representation: $I_{21}=I_\chi+I_{L_1\bks\chi}$.

For $I_{\chi}$, we only analysis the integral on $\chi_{21}=(t_1-\sigma,t_1+\sigma)$ denoted by $I^1_{\chi}$, the procedure of the another is same. Without loss of generality, we assume that $\sin (t_1-\sigma+\phi)<\kappa<\sin (t_1+\sigma+\phi)$. It is easy to see that $\sin (t+\phi)$ is monotonic increasing in $\chi_{21}$. Let $\sin (t+\phi) = \kappa \sin \theta$ and the implicit mapping from $\theta$ to $t$ is denoted by $t(\theta)$ while the inverse mapping by $\theta(t)$, taking the interval $\chi$ onto $L_\theta : \theta_1\rightarrow\pi/2\rightarrow\pi/2-\i\theta_2$ where $\sin(t_1-\sigma+\phi)=\kappa\sin \theta_1,\sin(t_1+\sigma+\phi)=\kappa\sin(\pi/2-\i\theta_2)$. By substituting $t(\theta)$ into $I^1_{\chi}$, we have
\be
I^1_{\chi}=\int_{L_\theta}\frac{ h(\kappa\sin\theta,(1-\kappa^2\sin^2\theta,\kappa\cos\theta)}{(1-\kappa^2\sin^2\theta)} \ e^{\i
...
\be
\Bigg|\frac{d^2\cos t}{d\theta^2}(0)\Bigg|=\frac{(1-\kappa^2)\kappa}{(1-\kappa^2)^{3/2}}|\sin t|>\frac{(1-\kappa^2)\kappa}{(1-\kappa^2)^{3/2}}\sin (\pi/2-\delta)
\ee
Therefore, we can choose appropriate a,b, only dependent on $\kappa$, such that $|\frac{d^2\cos t}{d\theta^2}|>\frac{(1-\kappa^2)\kappa}{(1-\kappa^2)^{3/2}}\sin (\pi/2-\delta)$ for any $\theta\in \theta(\chi_2)$. Since the amplitude function of integrand in $I^1_{\chi}$ and its derivative with respect to $\theta$ are both integrable on $L_\theta$, the estimation $|I^1_\chi|\leq C/(k_s\rho)^{1/2}$ can be obtained immediately by lemma \ref{van}.
Finally, an argument similar to the estimation of $I_{11}$ used shoes that $|I_{L_1\bks\chi}|\leq C/(k_s\rho)^{1/2}$. This completes the proof.
...
\begin{lem}\label{f_estimate}
	For any $z,y\in \Omega$,
	\be
	|F^{sp}_{ij}|\leq C(1+d_\Omega)(\frac{1}{z_2}+\frac{1}{z_2^{1/4}}+\frac{1}{z_2^{1/(2k_{c_3}-1)}})  \\
	|F^{ps}_{ij}|\leq C(1+d_\Omega)(\frac{1}{y_2}+\frac{1}{y_2^{1/4}}+\frac{1}{y_2^{1/(2k_{c_3}-1)}})
	\ee
where C may be only dependent on $\kappa$, $c_3$ and $k_{c_3}$ is the minimum of k which satisfy $\kappa^{2k-1}<1/c_3$.
\end{lem}
\debproof
...
\een
We only estimate $F^{ps}(z,y)$ here, the other terms can prove in the same way. let $\xi=k_s t$, we have
\ben
F^{ps}_{11}(z,y)=\frac{1}{\mu} \int^{\kappa}_{-\kappa} f(t,\kappa) e^{\i k_s t (y_1-z_1)} e^{\i k_s z_2\phi(t,\tau)} dt
\een
where $\phi(t,\tau)=(\sqrt{1-t^2}-\tau\sqrt{\kappa^2-t^2})$ and $\tau=y_2/z_2$. From the convention (\ref{o_bound}) we have $1/c_3<\tau<c_3$. A simple computation show that
\ben
...
\ee
 It is easy to see that $\phi^{(2k_{c_3})}(0,tau)\geq C(\frac{1}{c_3\kappa^{2k_{c_3}}}-1)$ due to $1/c_3<\kappa^{2k_{c_3}-1}$. Consequently, the estimation here can be proved by the same method as employed in the last case. This completes the proof.
\finproof

To complete the analysis of the point spread function, we define $F(z,y)=F_{ss}(z,y)+F_{pp}(z,y)$, where
\ben
\hspace{-2cm}
...
\ee
We are then left to proof $-\Im F^{ss2}_{11}>0$. If $\xi\in(-k_s,k_s)\bks(-k_p,k_p)$, $\mu_p=\i\sqrt(\xi-k_p^2)$. Substituting it into $F^{ss2}$, we have
\be
\hspace{-1.5cm}
...
\ben
-\Im F^{ss2}_{11}=\frac{1}{2\pi\mu}\int_{(-k_s,k_s)\bks(-k_p,k_p)} \frac{\mu_s^3(\xi^2-k_p^2)(k_s^2-\xi^2)^2}{|\alpha|^2} d\xi >0
\een
...
\begin{thm} \label{eu1}
	...
	\sigma(u)e_2=0 \ \ \ \ \mbox{\rm on} \Ga_0, \label{es3} \\
	 u \ \mbox{satisfies the generalized radiation codition such that} \nn \\\label{rc1}
...
\begin{lem}{\label{newton}}
	Let  $ f  \in L^2 (\R^2_+) $ with compact support in $B^2_+$. For any $z=1+\i\varepsilon$, $0<\varepsilon<1$, we have, for any $s>1/2$,
	$\|u_z\|_{H^{1,-s}(\R^2_+)}\le C\|f\|_{L^2(\R^2_+)}$ for some constant independent of $\varepsilon, u_z$, and $f$.
\end{lem}
\debproof
We define $R_z(f)=u_z$, then it is easy to see that $R_z$ is a linear bounded operator from $L^2_c(\R^2_+)$ to $H^{1,-s}(\R^2_+)$, where $L^2_c(\R^2_+)$ is denoted by all $ f  \in L^2 (\R^2_+) $ with compact support in $B_R$. It follows from theorem 3.7 in \cite{Yves1988} that $R_z$ is a uniformly continuous operator continues valued function on $z=1+\i\varepsilon$, $0<\varepsilon<1$ with value in $B(L^2_c(\R^2_+),H^{1,-s}(\R^2_+))$. Then, we can obtain that $R_z$ is uniformly bounded in $B(L^2_c(\R^2_+),H^{1,-s}(\R^2_+))$. This complete the proof by the defintion of the operator norm.
\finproof

We next recall the following lemma that respecting the absence of positive eigenvalues for the linear elasticity system in half space \cite{sini2004}.
...
outside of $B_{R_3}$. For $g\in H^{-1/2}(\Ga_D)$, let $u_g \in H^{1}(\R^2_+ \backslash \bar{D})$ be the lifting function such that $  u_{g}=g \mbox{ on } \Ga_D$ and $\|u_{g}\|_{H^{1}(\R^2_+ \backslash \bar{D})}\le C\|g\|_{H^{1/2}(\Ga_D)}$ hold. By testing \ref{wg2} with
$\chi_1^2 ( \overline{u_{\eps}-u_{g}} )$, we have by the standard argument
\be \label{u1}
...
Now we claim
\be \label{u3}
\|u_{\eps}\|_{H^{1,-s}(\R^2_+ \backslash \bar D)} \leq C \|g\|_{H^{1/2}(\Ga_D)} ,
\ee
...
\be {\label{contradict}}
\|u_{\eps_m}\|_{H^{1,-s}(\R^2_+ \backslash \bar D)} = 1 \ {\rm{ and }} \ \|g_m\|_{H^{-1/2}(\Ga_D)} \leq \frac{1}{m}.
\ee
Then $\|u_{\eps_m}\|_{H^{1}(B_{R_3} \backslash \bar D)}\le C $, and thus there is a subsequence of $\{\eps_m\}$, which is
still denoted by $\{\eps_m\}$, such that $\eps_m \to \eps' \in [0,1]$, and a subsequence of $\{u_{\eps_m}\}$,
which is still denoted by $\{u_{\eps_m}\}$, such that it converges to some $u_{\eps'} \in L^2(B_{R_3} \backslash \bar D)$ by Rellich imbedding theorem. Thus $u_{\eps_m}$ and $g_m$ are Cauchy sequeces respected in $L^2(B_{R_3} \backslash \bar D)$ and $H^{1/2}(\Ga_D)$. By extending inequality (\ref{u2}), we obtain that $u_{\eps_m}$ is also a Cauchy sequeces respected in $H^{1,-s}(\R^2_+ \backslash \bar D)$. So $u_{\eps'} \in H^{1,-s}(\R^2_+ \backslash \bar D)$ satisfies (\ref{wg2}-\ref{wg4}) with $g=0$ and $\eps=\eps'$.

By the integral representation satistied by $u_{\eps_m}$, we know that for $y\in \R^2_+\backslash\bar D$ and $i=1,2$
\be \label{ge1} \hspace{-2cm}
u_{\eps_m}(y)\cdot e^i=\int_{\Ga_D} (\sigma(G_{\eps_m}(x,y)e_i)\nu)\cdot u_{\eps_m}(x) - (G_{\eps_m}(x,y)e_i)\cdot (\sigma(u_{\eps_m})_{\eps_m}\nu)ds(x)
\ee
By the trace theorem, it is easy to see that $u_{\eps_m}\to u_{\eps'}$ in $H^{1/2}(\Ga_D)$ and $\sigma(u_{\eps_m})\to\sigma(u_{\eps'})$ in $H^{-1/2}(\Ga_D)$ when $\eps_m\to\eps'$. Then let $\eps_m\to\eps'$ in
(\ref{ge1}), we obtain that or $y\in \R^2_+\backslash\bar D$ and $i=1,2$
\be \ \hspace{-2cm} \label{ge2}
u_{\eps'}(y)\cdot e^i=\int_{\Ga_D} (\sigma(G_{\eps'}(x,y)e_i)\nu)\cdot u_{\eps'}(x) - (G_{\eps'}(x,y)e_i)\cdot (\sigma(u_{\eps'})_{\eps'}\nu)ds(x)
\ee
...
\begin{lem} \label{ext}
	For any $s>1/2$, $u_\eps:(0,1)\to H^{1,-s}(\R^2_+ \bks \bar D)$ is a uniformly continuous operator valued function. Immediately, $u_{\eps}$ converges to some $u$ in $H^{1,-s}(\R^2_+ \bks \bar D)$ and $u_0$ is a solution of (\ref{es1}-\ref{es4}).
\end{lem}
\debproof
...
\ee
Thus there is a subsequence of $\{\mu_n\}$, which is still denoted by $\{\mu_n\}$, such that $\{\mu_n\}\to \epsilon\in[0,1]$ and also $\{\nu_n\}\to \epsilon$. Then using lemma \ref{newton2} and the procedure proving it, we get the $u_\epsilon,v_\epsilon \in H^{1,-s} (\R^2_+ \bks \bar D)$ such that
\ben
...
\een
Then the desired unique exsitence follows with lemma \ref{ext}. This comlpletes the proof of theorem \ref{eu1}.
\finproof

\section{The resolution analysis}
In this section we study the imaging resolution of the RTM for the Direchlet boundary obstacle in the half space.
