...
For the claim 1, wo only give the proof when $\phi\in(\phi_2,\pi/2)$ since the similar proof can be adjusted to the other case. By the convention of $\phi_2$, for any $\phi\in(\phi_2,\pi/2)$, there exists $0<\delta<\pi/4$ only dependent on $\kappa$ such that $|\sin(t+\phi)|>(1+2\kappa)/3, |\cos(t+\phi)|<2\kappa/3$ for any $t\in(-\delta,\delta)$ while $|\cos(t+\phi)|>(1+2\kappa)/3, |\sin(t+\phi)|<2\kappa/3$ for any $t\in(-\pi/2,-\pi/2+\delta)\cup(\pi/2-\delta,\pi/2)$. Let $\chi_\delta\in C^\infty_0(-\pi/2,\pi/2)$ be the cut-off function with that $0\leq\chi_\delta\leq1$, $\chi_\delta=1$ in $(-\delta/2,\delta/2)$ and $\chi_\delta=0$ outside of $(-\delta,\delta)$. Then we can divide $I_1$ into three parts such that
\be
...
+\int_{L_2} f(t)\cos t e^{\i k_s\rho\cos t}dt \\
=: I_{11} + I_{12} +I_{13}
\ee
where $L_1=(-\pi/2,\pi/2)$ and $L_2=(-\pi/2+\i\infty,-\pi/2)\cup(\pi/2,\pi/2-\i\infty)$.

The proof of the claim 2 is same to the claim1, here we omit the details.
\finproof
