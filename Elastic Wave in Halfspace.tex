...
which is still denoted by $\{u_{\eps_m}\}$, such that it converges to some $u_{\eps'} \in {H^{1,-s'} ( \R^2_+ \backslash \bar{D})}$ by Rellich imbedding theorem where $s<s'$. Thus $u_{\eps_m}$ and $g_m$ are Cauchy sequeces respected in $L^2(B_{R_3} \backslash \bar D)$ and $H^{1/2}(\Ga_D)$. So $u_{\eps'} \in H^{1,-s'}(\R^2_+ \backslash \bar D)$ satisfies (\ref{wg2}-\ref{wg4}) with $g=0$ and $\eps=\eps'$.
...
(\ref{ge1}), we obtain that for $y\in \R^2_+\backslash\bar D$ and $i=1,2$
