...
\begin{lem} \label{asym_frac}
	...
	\be
	|F(\lambda,a)|\leq C(\frac{1}{\lambda^\alpha}+\frac{1}{\lambda}(a^{\alpha-1}f(a)+|t^{\alpha-1}f'(t)|_{L^1(0,a)})
	\ee
\end{lem}
\begin{lem}\label{pr_dgreen}
	...
	\\ \hspace{-0.5cm}
	\leq C(\frac{k_p b}{\rho(k_p\rho)^{3/4}}+\frac{k_p a}{\rho(k_p\rho)^{5/4}})
	\ee
	where C is only dependent on $\kappa$.
\end{lem}
\debproof
...
2. For the claim 2, since $\phi\neq\phi_1\kappa$ we always can find some $\delta$ enough small such that $\sin^(t+\phi)\neq \kappa^2$ for any $t\in(-\delta,\delta)$ .Thus, the proof is similar to the claim1, here we omit the details.

3. To prove the claim 3, observe that 0 and $\phi'$ are the only two movable singular points of $f'(t)$ for $t\in L$ where $\sin^2{(\phi'+\phi_\kappa)}=\kappa^2$ and $\phi'\neq 0$.
However, we can not use stationary phase lemma directly because the fourth derivatives of amplitude function has singularity when $t=0$. Let $0<\delta<\pi/4$ such that $\sin (t+\phi_{\kappa})$ is monotonic as $t\in(-\delta,\delta)$, then we have $I_1=I_{11}+I_{12}$,
$I_3=I_{31}+I_{32}$ like (\ref{I_split}). Using the same argument as in the proof of the claim 1, we easily obtain 
\be
|I_{12}|\leq C\frac{1}{k_s \rho} \ \ \ \  |I_{32}|\leq C\frac{1}{(k_s \rho)^{1/2}}
\ee
Obseve that we can always obtain following representation of $f(t)$
\be
